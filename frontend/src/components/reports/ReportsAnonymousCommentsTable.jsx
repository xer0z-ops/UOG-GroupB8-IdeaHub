...
function ReportsAnonymousCommentsTable({ title, rows = [], onDetail }) {
        ...
        <>
          <TableContainer>
            <Table>
              <TableHead>
                <TableRow>
                  {['Comment', 'Title', 'Department', 'Category', 'Tmb Up/Down/Rep', 'Actions'].map((header) => (
                    <TableCell
                      key={header}
                      sx={{
                        fontWeight: 700,
              ...
              <TableBody>
                {pageItems.map((item) => (
                  <TableRow key={item.id} hover sx={{ '& td': { borderBottom: '1px solid #f2f4f9' } }}>
                    <TableCell sx={{ color: '#4a5667' }}>{item.comment}</TableCell>
                    <TableCell sx={{ textAlign: 'center', color: '#4a5667' }}>{item.title}</TableCell>
...
export default ReportsAnonymousCommentsTable;
